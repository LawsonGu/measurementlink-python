...
import grpc
from ni_measurement_service._internal.stubs import HealthCheck_pb2
from ni_measurement_service._internal.stubs import HealthCheck_pb2_grpc

# The service name of the health checking servicer.
SERVICE_NAME = HealthCheck_pb2.DESCRIPTOR.services_by_name['Health'].full_name
...
class HealthServicer(HealthCheck_pb2_grpc.HealthServicer):
    ...
    def __init__(self,
        ...
        self._server_status = {"": HealthCheck_pb2.HealthCheckResponse.SERVING}
    ...
    def Check(self, request, context):
        with self._lock:
            ...
            if status is None:
                ...
                return HealthCheck_pb2.HealthCheckResponse(status=status)
    ...
    def Watch(self, request, context, send_response_callback=None):
        ...
        with self._lock:
            ...
            if status is None:
                status = HealthCheck_pb2.HealthCheckResponse.SERVICE_UNKNOWN  # pylint: disable=no-member
            send_response_callback(
                HealthCheck_pb2.HealthCheckResponse(status=status))
        ...
        with self._lock:
            if self._gracefully_shutting_down:
                ...
                for service in self._server_status:
                    self.set(service,
                             HealthCheck_pb2.HealthCheckResponse.NOT_SERVING)  # pylint: disable=no-member
